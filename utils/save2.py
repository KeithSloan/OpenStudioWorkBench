...
def generate_feature_class(xsd_file, element_name):
    ...
    for sub in subelements:
        ...
        if prop_name is None:
            # Skip elements without an identifiable name
            continue
        ...
        if xsd_type is None:
            ...
            if simpleType is not None:
                ...
                if restriction is not None:
                    ...
                    if base:
                        # Remove any namespace prefix and lower-case the type
                        xsd_type = base.split(':')[-1].lower()
                    enums = restriction.findall('xs:enumeration', namespaces=ns)
                    if enums:
                        enum_values = [e.get('value') for e in enums]
        else:
            # Remove namespace prefix if any and lower-case the type
            xsd_type = xsd_type.split(':')[-1].lower()
    ...
    # Join all the lines using a newline separator.
    generated_code = "\n".join(lines)
