...
complex types with simpleContent extensions (where a base value and attributes are defined).
It then outputs a Python file named <element_name>_Feature.py. The generated code defines
a class that, when attached to a FreeCAD FeaturePython object, creates properties (mapping
XSD types to FreeCAD properties) and provides a Qt dialog (using PySide2) to edit those values.
"""
...
def generate_feature_class(xsd_file, element_name):
    ...
    # Check if the element uses simpleContent (extension) style.
    ...
    if simpleContent is not None:
        ...
        if extension is None:
            ...
            sys.exit(1)
        # Process the base type for the main value.
        base = extension.get("base")
        if base:
            base_type = base.split(':')[-1].strip().lower()
            # Resolve custom types if needed.
            if base_type not in XSD_TO_FC_MAP:
                base_type = resolve_xsd_type(base_type, root, ns)
        else:
            base_type = 'string'
        # Create a property named "Value" for the main content.
        fc_prop_type = XSD_TO_FC_MAP.get(base_type, 'App::PropertyString')
        properties.append(("Value", fc_prop_type, None))
        
        # Process attributes of the extension.
        for attr in extension.findall("xs:attribute", namespaces=ns):
            ...
            properties.append((attr_name, fc_prop_type_attr, enum_values))
    else:
    ...
    for prop_name, fc_prop_type, enum_values in properties:
        ...
        if fc_prop_type == "App::PropertyEnumeration" and enum_values is not None:
            ...
            # Assume a custom method to set enumeration values exists.
