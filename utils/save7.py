...
specified name. It handles both complex types with sequences of subelements and
complex types with simpleContent extensions.
For simpleContent, it creates a property with the element’s name (e.g. “Area”)
for the base value (mapped from xsd:decimal to a float) and, for attributes (such as
unit), it renames them (e.g. “unit” becomes “AreaType”) and maps to a FreeCAD enumeration.
...
# Note: since FreeCAD doesn't support a decimal property, we map decimal to float.
XSD_TO_FC_MAP = {
...
def resolve_xsd_type(custom_type, root, ns):
    ...
    Searches for a global xs:simpleType with name == custom_type.
...
def generate_feature_class(xsd_file, element_name):
    ...
    # Check if the element uses a simpleContent extension.
    simpleContent = elem.find("xs:complexType/xs:simpleContent", namespaces=ns)
    if simpleContent is not None:
        extension = simpleContent.find("xs:extension", namespaces=ns)
        if extension is None:
            print(f"Element '{element_name}' has simpleContent but no extension found.")
            sys.exit(1)
        # Process the base type.
        base = extension.get("base")
        if base:
            base_type = base.split(':')[-1].strip().lower()
            # Force "decimal" to be treated as float.
            if base_type == "decimal":
                base_type = "float"
            # Resolve custom types if not in our mapping.
            if base_type not in XSD_TO_FC_MAP:
                base_type = resolve_xsd_type(base_type, root, ns)
        else:
            base_type = 'string'
        fc_prop_type = XSD_TO_FC_MAP.get(base_type, 'App::PropertyString')
        # Use the element name (e.g. "Area") as the property name.
        properties.append((element_name, fc_prop_type, None))
        
        # Process attributes defined in the extension.
        for attr in extension.findall("xs:attribute", namespaces=ns):
            attr_name = attr.get("name")
            # Rename "unit" to "AreaType" (or elementName + "Type")
            if attr_name == "unit":
            ...
            if attr_type:
                ...
                if attr_type not in XSD_TO_FC_MAP:
                    attr_type = resolve_xsd_type(attr_type, root, ns)
            else:
                attr_type = 'string'
            # If the attribute type name ends with "enum", force an enumeration property.
        ...
        # Fallback: process subelements from a sequence.
        subelements = elem.xpath(".//xs:element", namespaces=ns)
