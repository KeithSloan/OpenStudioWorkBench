...
# (FreeCAD doesn’t offer a decimal type, so we map decimal to float)
XSD_TO_FC_MAP = {
...
def resolve_xsd_type(custom_type, root):
    ...
    """
    # Use local-name() to find the simpleType regardless of prefix.
...
def generate_feature_class(xsd_file, element_name):
    ...
    # Use local-name() for namespace agnostic search.
    ...
    simpleContent = elem.find("*[local-name()='complexType']/*[local-name()='simpleContent']")
    if simpleContent is not None:
        extension = simpleContent.find("*[local-name()='extension']")
        ...
        for attr in extension.findall("*[local-name()='attribute']"):
            ...
            if attr_type:
                ...
                attr_type = 'string'
            # If the type name ends with "enum", force an enumeration property.
